function S(x1,y1,x2,y2,x3,y3) {
	...
	if (cos132 <= 0) return -1;
	
	var k = 0;
	
	if (Math.abs(cos213) < Math.PI && Math.abs(cos123) < Math.PI && Math.abs(cos132) < Math.PI) {
		var p = (L12+L13+L23)/2;
		var ss = Math.sqrt(p*(p-L12)*(p-L13)*(p-L23));
		return ss;
	}
}
